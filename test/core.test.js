...
// burnEffort / zone cue / item strip / menu weeks
{
  ...
  assert.strictEqual(C.burnShowsZoneCue({ hard: true, zoneCue: false }, 4), false);
...
// Live program: W1–6 explicit, W3 D2 ≠ W1, only D4 W5 is a benchmark
{
  ...
  const d4w5 = C.resolveBurn(P.days[3], 5);
  assert.ok(d4w5.benchmark, "D4 W5 stays the sole benchmark");
  assert.ok(/get-to-stand/i.test((d4w5.items || []).join(" ")));
  assert.ok(!/burpee|goblet squat/i.test((d4w5.items || []).join(" ")));
  ...
  assert.ok(/box jump/i.test(elasticW5.rx), "W5 D4 elastic owned option includes box jump with step-down");
  assert.ok(/71|5'11/.test(elasticW5.note), "W5 D4 broad jump notes the 5'11 / 71 in standard");
  assert.ok(!/slam/i.test(elasticW5.rx + " " + elasticW5.nm), "W5 D4 elastic must not offer slams");
  ...
  assert.ok(/8–10 min/.test(C.resolveBurn(P.days[1], 4).fmt));
  assert.ok(!/^\d+$/.test(C.burnFmtChip(C.resolveBurn(P.days[1], 4).fmt)), "Tuesday chip must still parse");
